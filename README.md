...
| 专注模式 | 支持正计时与倒计时，记录单次专注过程；倒计时结束后通过循环音效、Windows 通知和完成弹窗持续提醒。 |
| 学习记录 | 管理历史学习记录，支持按阶段、分类、日期和内容追踪学习过程。 |
| 统计分析 | 通过趋势、分类、阶段和效率数据观察学习投入结构。 |
| AI 助手 | 通过 OpenAI 兼容接口进行多轮对话，由模型按问题读取本地学习数据并生成分析。 |
...
| 数据管理 | 支持本地数据导出、导入和清空。 |
| AI 设置 | 管理 OpenAI 兼容接口地址、API Key 和模型列表。 |
...
- AI 助手按对话问题读取本机 SQLite 数据库中的学习记录、阶段、分类、倒计时、成就和课程画像数据。
- AI 设置保存 OpenAI 兼容接口地址、API Key 和模型名称，模型列表可通过接口自动刷新。
